 ...
 */
?>
@extends('layouts.app', ['title' => __('data_tempat_dilahirkan') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_tempat_dilahirkan') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_tempat_dilahirkan') }}</div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                @can('create_data_tempat_dilahirkan')
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_tempat_dilahirkan') }}
                                </button>
                                @endcan

                                @can('trash_data_tempat_dilahirkan')
                            ...
                            <form action="{{ route('data-tempat-dilahirkans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-tempat-dilahirkans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-tempat-dilahirkans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_tempat_dilahirkan')
<!-- modal add -->@include('pages.data-tempat-dilahirkans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_tempat_dilahirkan')
<!-- modal edit -->@include('pages.data-tempat-dilahirkans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-tempat-dilahirkans.modal-detail')
<!-- end modal detail -->
@can('delete_data_tempat_dilahirkan')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-tempat-dilahirkans.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-tempat-dilahirkans.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-tempat-dilahirkans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
