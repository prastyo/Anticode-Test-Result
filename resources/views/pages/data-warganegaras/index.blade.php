 ...
 */
?>
@extends('layouts.app', ['title' => __('data_warganegara') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_warganegara') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_warganegara') }}</div>
            </div>
        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_warganegara') }}
                                </button>
                                @endcan

                                @can('trash_data_warganegara')
                            ...
                            <form action="{{ route('data-warganegaras.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-warganegaras.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-warganegaras.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_warganegara')
<!-- modal add -->@include('pages.data-warganegaras.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_warganegara')
<!-- modal edit -->@include('pages.data-warganegaras.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-warganegaras.modal-detail')
<!-- end modal detail -->
@can('delete_data_warganegara')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-warganegaras.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-warganegaras.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-warganegaras.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
