 ...
 */
?>
@extends('layouts.app', ['title' => __('data_asuransi') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_asuransi') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_asuransi') }}</div>
            </div>
        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_asuransi') }}
                                </button>
                                @endcan

                                @can('trash_data_asuransi')
                            ...
                            <form action="{{ route('data-asuransis.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-asuransis.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-asuransis.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_asuransi')
<!-- modal add -->@include('pages.data-asuransis.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_asuransi')
<!-- modal edit -->@include('pages.data-asuransis.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-asuransis.modal-detail')
<!-- end modal detail -->
@can('delete_data_asuransi')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-asuransis.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-asuransis.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-asuransis.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
