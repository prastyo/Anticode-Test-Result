 ...
 */
?>
@extends('layouts.app', ['title' => __('data_hubungan_keluarga') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_hubungan_keluarga') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_hubungan_keluarga') }}</div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                @can('create_data_hubungan_keluarga')
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_hubungan_keluarga') }}
                                </button>
                                @endcan

                                @can('trash_data_hubungan_keluarga')
                            ...
                            <form action="{{ route('data-hubungan-keluargas.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-hubungan-keluargas.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-hubungan-keluargas.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_hubungan_keluarga')
<!-- modal add -->@include('pages.data-hubungan-keluargas.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_hubungan_keluarga')
<!-- modal edit -->@include('pages.data-hubungan-keluargas.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-hubungan-keluargas.modal-detail')
<!-- end modal detail -->
@can('delete_data_hubungan_keluarga')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-hubungan-keluargas.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-hubungan-keluargas.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-hubungan-keluargas.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
