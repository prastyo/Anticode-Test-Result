 ...
 */
?>
@extends('layouts.app', ['title' => __('data_pendidikan') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_pendidikan') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_pendidikan') }}</div>
            </div>
        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_pendidikan') }}
                                </button>
                                @endcan

                                @can('trash_data_pendidikan')
                            ...
                            <form action="{{ route('data-pendidikans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-pendidikans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-pendidikans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_pendidikan')
<!-- modal add -->@include('pages.data-pendidikans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_pendidikan')
<!-- modal edit -->@include('pages.data-pendidikans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-pendidikans.modal-detail')
<!-- end modal detail -->
@can('delete_data_pendidikan')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-pendidikans.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-pendidikans.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-pendidikans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
