                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_bahasa') }}
                                </button>
                                @endcan

                                @can('trash_data_bahasa')
                            ...
                            <form action="{{ route('data-bahasas.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-bahasas.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-bahasas.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_bahasa')
<!-- modal add -->@include('pages.data-bahasas.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_bahasa')
<!-- modal edit -->@include('pages.data-bahasas.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-bahasas.modal-detail')
<!-- end modal detail -->
@can('delete_data_bahasa')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-bahasas.datatables') }}',
        type: 'POST',
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-bahasas.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
