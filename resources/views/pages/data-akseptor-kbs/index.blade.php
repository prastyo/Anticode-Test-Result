 ...
 */
?>
@extends('layouts.app', ['title' => __('data_akseptor_kb') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_akseptor_kb') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_akseptor_kb') }}</div>
            </div>
        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_akseptor_kb') }}
                                </button>
                                @endcan

                                @can('trash_data_akseptor_kb')
                            ...
                            <form action="{{ route('data-akseptor-kbs.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-akseptor-kbs.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-akseptor-kbs.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_akseptor_kb')
<!-- modal add -->@include('pages.data-akseptor-kbs.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_akseptor_kb')
<!-- modal edit -->@include('pages.data-akseptor-kbs.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-akseptor-kbs.modal-detail')
<!-- end modal detail -->
@can('delete_data_akseptor_kb')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-akseptor-kbs.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-akseptor-kbs.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-akseptor-kbs.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
