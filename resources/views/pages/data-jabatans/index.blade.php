                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_jabatan') }}
                                </button>
                                @endcan

                                @can('trash_data_jabatan')
                            ...
                            <form action="{{ route('data-jabatans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-jabatans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-jabatans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_jabatan')
<!-- modal add -->@include('pages.data-jabatans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_jabatan')
<!-- modal edit -->@include('pages.data-jabatans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-jabatans.modal-detail')
<!-- end modal detail -->
@can('delete_data_jabatan')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-jabatans.datatables') }}',
        type: 'POST',
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-jabatans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
