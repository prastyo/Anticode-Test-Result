                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_kursus') }}
                                </button>
                                @endcan

                                @can('trash_data_kursus')
                            ...
                            <form action="{{ route('data-kursuses.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-kursuses.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-kursuses.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_kursus')
<!-- modal add -->@include('pages.data-kursuses.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_kursus')
<!-- modal edit -->@include('pages.data-kursuses.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-kursuses.modal-detail')
<!-- end modal detail -->
@can('delete_data_kursus')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-kursuses.datatables') }}',
        type: 'POST',
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-kursuses.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
