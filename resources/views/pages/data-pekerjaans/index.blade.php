 ...
 */
?>
@extends('layouts.app', ['title' => __('data_pekerjaan') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_pekerjaan') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_pekerjaan') }}</div>
            </div>
        </div>
                ...
                <div class="card card-primary">
                    <div class="card-body">
                        <div class="mb-3 row">
                            ...
                            <form action="{{ route('data-pekerjaans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-pekerjaans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-pekerjaans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_pekerjaan')
<!-- modal add -->@include('pages.data-pekerjaans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_pekerjaan')
<!-- modal edit -->@include('pages.data-pekerjaans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-pekerjaans.modal-detail')
<!-- end modal detail -->
@can('delete_data_pekerjaan')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-pekerjaans.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-pekerjaans.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-pekerjaans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
