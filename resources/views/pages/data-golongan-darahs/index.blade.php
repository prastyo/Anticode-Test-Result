 ...
 */
?>
@extends('layouts.app', ['title' => __('data_golongan_darah') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_golongan_darah') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_golongan_darah') }}</div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                @can('create_data_golongan_darah')
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_golongan_darah') }}
                                </button>
                                @endcan

                                @can('trash_data_golongan_darah')
                            ...
                            <form action="{{ route('data-golongan-darahs.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-golongan-darahs.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-golongan-darahs.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_golongan_darah')
<!-- modal add -->@include('pages.data-golongan-darahs.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_golongan_darah')
<!-- modal edit -->@include('pages.data-golongan-darahs.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-golongan-darahs.modal-detail')
<!-- end modal detail -->
@can('delete_data_golongan_darah')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-golongan-darahs.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-golongan-darahs.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-golongan-darahs.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
