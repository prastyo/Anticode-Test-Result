    ...
    <div class="modal-dialog modal-lg modal-dialog-scrollable modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title mx-auto">{{ __('site.edit') }} {{ __('users') }}</h5>
    ...
    $('#modal_edit').on('shown.bs.modal', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                response.data.roles.forEach(element => { $(modale).find('#role_name').val(element.name); });
                $(modale).find('#name').val(response.data.name);
                $(modale).find('#username').val(response.data.username);
                $(modale).find('#email').val(response.data.email);
                $(modale).find('#bio').val(response.data.bio);
                $(modale).find('#password').val(response.data.password);
