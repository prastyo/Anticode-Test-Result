 ...
 */
?>
@extends('layouts.app', ['title' => __('data_sakit_menahun') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_sakit_menahun') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_sakit_menahun') }}</div>
            </div>
        </div>
                        ...
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                ...
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_sakit_menahun') }}
                                </button>
                                @endcan

                                @can('trash_data_sakit_menahun')
                            ...
                            <form action="{{ route('data-sakit-menahuns.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-sakit-menahuns.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-sakit-menahuns.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_sakit_menahun')
<!-- modal add -->@include('pages.data-sakit-menahuns.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_sakit_menahun')
<!-- modal edit -->@include('pages.data-sakit-menahuns.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-sakit-menahuns.modal-detail')
<!-- end modal detail -->
@can('delete_data_sakit_menahun')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-sakit-menahuns.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-sakit-menahuns.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-sakit-menahuns.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
