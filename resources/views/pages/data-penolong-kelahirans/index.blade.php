 ...
 */
?>
@extends('layouts.app', ['title' => __('data_penolong_kelahiran') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_penolong_kelahiran') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_penolong_kelahiran') }}</div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                @can('create_data_penolong_kelahiran')
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_penolong_kelahiran') }}
                                </button>
                                @endcan

                                @can('trash_data_penolong_kelahiran')
                            ...
                            <form action="{{ route('data-penolong-kelahirans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-penolong-kelahirans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-penolong-kelahirans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_penolong_kelahiran')
<!-- modal add -->@include('pages.data-penolong-kelahirans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_penolong_kelahiran')
<!-- modal edit -->@include('pages.data-penolong-kelahirans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-penolong-kelahirans.modal-detail')
<!-- end modal detail -->
@can('delete_data_penolong_kelahiran')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-penolong-kelahirans.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-penolong-kelahirans.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-penolong-kelahirans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
