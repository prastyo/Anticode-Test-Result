 ...
 */
?>
@extends('layouts.app', ['title' => __('data_jenis_persalinan') ])
@section('content')
<div id="#section-to-print"></div>
<div class="main-content">
    <section class="section">
        <div class="section-header">
            <h1>{{ __('data_jenis_persalinan') }}</h1>
            <div class="section-header-breadcrumb">
                <div class="breadcrumb-item active"><a href="#">{{ __('site.dashboard') }}</a></div>
                <div class="breadcrumb-item">{{ __('data_jenis_persalinan') }}</div>
            </div>
        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3 row">
                            <div class="col-sm-12 col-md-6">
                                @can('create_data_jenis_persalinan')
                                <button type="button" class="btn btn-icon icon-left btn-primary mr-2"
                                    ...
                                    {{ __('site.add') }} {{ __('data_jenis_persalinan') }}
                                </button>
                                @endcan

                                @can('trash_data_jenis_persalinan')
                            ...
                            <form action="{{ route('data-jenis-persalinans.excel') }}" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-jenis-persalinans.print') }}" target="_blank" method="POST">
                                    @csrf
                                ...
                                <form action="{{ route('data-jenis-persalinans.pdf') }}" method="POST">
                                    @csrf
...
@can('create_data_jenis_persalinan')
<!-- modal add -->@include('pages.data-jenis-persalinans.modal-add')
<!-- end modal add -->
@endcan

@can('edit_data_jenis_persalinan')
<!-- modal edit -->@include('pages.data-jenis-persalinans.modal-edit')
<!-- end modal edit -->
@endcan

<!-- modal detail -->@include('pages.data-jenis-persalinans.modal-detail')
<!-- end modal detail -->
@can('delete_data_jenis_persalinan')
...
var table = $('#table_datatables').DataTable({
    ...
    ajax: {
        url: '{{ route('data-jenis-persalinans.datatables') }}',
        type: 'POST',
...
$('#button_delete_permanently').click(function() {
    $.ajax({
        url: '{{ route('data-jenis-persalinans.purge') }}',
        type: "POST",
        data: {
...
$('#button_restore').click(function() {
    $.ajax({
        url: '{{ route('data-jenis-persalinans.restore') }}',
        type: "POST",
        data: {
                id: $('#modal_detail').find('#id').val()
        },
