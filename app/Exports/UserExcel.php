...
class UserExcel implements FromQuery, WithHeadings, WithMapping, ShouldAutoSize, WithEvents, WithColumnFormatting, WithProperties
{
    use Exportable;
...
/*
        ...
        return User::query()query()->where([
            ['name', 'LIKE', '%' . $this->search_query . '%'],
            ['username', 'LIKE', '%' . $this->search_query . '%'],
            ['email', 'LIKE', '%' . $this->search_query . '%'],
            ['bio', 'LIKE', '%' . $this->search_query . '%']
        ])
